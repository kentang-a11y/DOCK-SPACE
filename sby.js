...
function deleteShip(index) {
    let ships = JSON.parse(localStorage.getItem('ships_sby')) || [];
    ships.splice(index, 1); // Remove the ship at the specified index
    localStorage.setItem('ships_sby', JSON.stringify(ships)); // Update local storage
...
document.getElementById('deleteAllButton').addEventListener('click', function() {
    localStorage.removeItem('ships_sby'); // Clear all ships from local storage
