...
document.getElementById('dockingForm').addEventListener('submit', function(event) {
    ...
    if (canDock(loa, b, stayStart, stayEnd)) {
        totalLengthUsed += loa; // Update total length used

        // Save ship details to local storage
        const shipDetails = {
            shipName: shipName,
            loa: loa,
            beam: b,
        ...
        // Retrieve existing ships from local storage
...
function displayShipList(ships) {
    ...
    if (ships.length === 0) {
        shipTableBody.innerHTML = '<tr><td colspan="8">No ships docked yet.</td></tr>';
        return;
    }
    ...
    ships.forEach((ship, index) => {
        ...
        row.innerHTML = `
            ...
            <td><button class="delete-button" onclick="deleteShip(${index})">Del</button></td>
        `;
...
function populateCalendarSchedule() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        const usedLength = calculateTotalLengthUsedForDate(date);
        const remainingLength = dockLength - usedLength;
        const percentageOccupied = (usedLength / dockLength) * 100;

        // Create a cell for the current date
        const cell = document.createElement('td');
        cell.innerHTML = `
            <strong>${day}</strong><br>
            Remaining: ${remainingLength.toFixed(2)} m<br>
            Occupied: ${percentageOccupied.toFixed(2)} %
        `;
        ...
        // Apply color gradation based on percentage occupied
if (percentageOccupied === 0) {
    cell.style.backgroundColor = 'white'; // 0% - Empty
} else if (percentageOccupied < 60) {
    cell.style.backgroundColor = 'lightgreen'; // Below 60% - Green
} else {
    cell.style.backgroundColor = 'lightblue'; // 60% or above - Red
}
